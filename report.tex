...
\hypersetup{
    ...
    filecolor=magenta,      
    urlcolor=blue,
    pdftitle={VPN Implementation Report},
}
...
% Title Information
\title{\textbf{VPN Implementation with Virtual Network Functions}}
\author{
    \textbf{Prateek Rath} \\  IMT2022017
    \and 
    \textbf{Mohit Naik} \\ IMT2022076
    \and 
    \textbf{Ananthakrishna K} \\ IMT2022086
}
\date{}
...
This is a simple FastAPI application which is not externally exposed. It has just one functional endpoint \texttt{/status}, which returns an \texttt{OK} when requested.
...
% NOTE: Make sure the file 'assets/arch.png' exists in your directory, or comment this block out.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{assets/arch.png}
    \caption{Architecture}
    \label{fig:architecture}
\end{figure}

\subsection{Containerization and Orchestration}
All the containerization and orchestration is present in the \texttt{k8s} folder. Broadly, we can divide our application's main functionality into:
\begin{itemize}
    \item the nginx reverse proxy
    \item the vpn gateway where auth takes place
    \item the app itself which handles requests.
\end{itemize}
...
Each deployment folder is armed with a \texttt{kustomization.yaml} file that simply allows us to use the \texttt{-k} flag and apply all yaml files within a folder via a single command.
...
\begin{itemize}
    \item Number of \texttt{/status} requests allowed by the vpn
    \item Number of api requests which fail due to invalid/expired JWT tokens
\end{itemize}
...
\begin{itemize}
    \item Logs per container bar chart
    \item Logs over time line chart
    \item Error Logs v/s Normal Logs over time
    \item Top 5 pods with the most error logs
    \item Log Volume by Namespace Pie chart
\end{itemize}

To view these:
\begin{enumerate}
    \item Deploy the application via the jenkins pipeline.
    \item Switch to the jenkins user by running \texttt{sudo su - jenkins}
    \item Run \texttt{kubectl port-forward svc/kibana 30090:5601 -n logging}
    \item Head to \url{http://localhost:5601} in the browser
    \item Click on $\equiv$ (menu) icon in the top-left corner
    \item Scroll to the bottom and click on Stack Management under the Management Section
    \item In Stack Management, click on Saved Objects under Kibana in the left pane
    \item Click import and then import the \texttt{kibana\_dashboard.ndjson} file present in the viz folder of the github repo
    \item Refresh the visualization to see real time updates.
\end{enumerate}
...
\section{Innovation}
\subsection{White-Box Application Telemetry}
Most standard Kubernetes setups only monitor infrastructure metrics like CPU and RAM. We innovated by implementing \textbf{White-Box Application Telemetry} directly within our VPN's Python code using the Prometheus client library.

We instrumented our application to expose internal logic states, specifically creating custom metrics for successful VPN tunnels and invalid authentication attempts. By visualizing these in Grafana, we transformed our dashboard into a security tool. A spike in the \texttt{vpn\_jwt\_invalid\_total} metric alerts us immediately to potential security incidents—such as token tampering or brute-force attacks that would otherwise be invisible to standard infrastructure monitoring tools.
...
ansible-playbook -i ./ansible/inventory.ini ./ansible/playbook.yml --vault-password-file ./ansible/password.txt
    ...
    \item The main app is intuitive to use, you can log in, check a request to \texttt{/status}, and send requests at a certain rate.
    \item Grafana will then show you visualizations of metrics scraped by Prometheus.
    \item First create a Data view with the pattern \texttt{filebeat-*}. Then, head to the Discover tab and filter logs as per requirements (e.g. \texttt{kubernetes.labels.app:app}). For a dashboard, detailed steps are mentioned in the logging section.
\end{enumerate}

\section{Authors}
\begin{itemize}
    \item Prateek Rath (IMT2022017)
    \item Mohit Naik (IMT2022076)
    \item Ananthakrishna K (IMT2022086)
\end{itemize}
